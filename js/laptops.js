// Генерация массива ноутбуков
const brands = ["Apple", "Asus", "HP", "Lenovo", "Acer", "Dell", "MSI", "Huawei", "Samsung", "Xiaomi"];
const laptops = Array.from({length: 60}, (_, i) => {
    const brand = brands[Math.floor(Math.random() * brands.length)];
    return {
        id: i+1,
        name: `${brand} Laptop ${2025 - (i % 5)} (${i+1})`,
        brand,
        price: Math.floor(39990 + Math.random() * 150000),
        image: 'images/macbook.png',
        desc: `Современный ноутбук ${brand} с мощным процессором и длительной автономностью.`
    };
});
...
function getLaptopSpecs(product) {
    // Примерные характеристики для ноутбука
    return [
        ['Экран', '15.6" IPS, 120 Гц'],
        ['Процессор', 'Intel Core i7 / AMD Ryzen 7'],
        ['Память', '16 ГБ ОЗУ, 512 ГБ SSD'],
        ['Видеокарта', 'NVIDIA RTX 4060 / Intel Iris Xe'],
        ['Батарея', '70 Вт*ч'],
        ['ОС', 'Windows 11 / macOS'],
        ['Вес', '1.5 кг'],
        ['Бренд', product.brand],
...
function showProductModal(product) {
    ...
    const specs = getLaptopSpecs(product);
    
    // Создаем хайку описание для ноутбука
    const haikuDesc = generateLaptopHaiku(product);
...
function generateLaptopHaiku(product) {
    const haikus = [
        'Тихий гул вентиляторов — шепот цифрового ветра.<br>Процессор Intel i7, память 16 ГБ, срок доставки — 3 луны.',
        'Утренний свет — отражение на алюминиевом корпусе.<br>Экран 15.6" IPS, SSD 512 ГБ, срок доставки — 2 луны.',
        'Вечерняя работа — тени на клавиатуре танцуют.<br>Видеокарта RTX 4060, батарея 70 Вт*ч, срок доставки — 4 луны.',
        'Лунный свет — тишина в спящем режиме.<br>Процессор AMD Ryzen, память 32 ГБ, срок доставки — 1 луна.',
        'Капля росы — конденсат на металлической поверхности.<br>Экран 4K OLED, вес 1.5 кг, срок доставки — 3 луны.'
    ];
